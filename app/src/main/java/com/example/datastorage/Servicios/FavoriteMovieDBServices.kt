...
class FavoriteMovieDBServices(context: Context) : SQLiteOpenHelper(context, "FavoriteMovieDBServices", null, 1), IFavoriteMovieServices
{
    ...
    fun restoreDB(db : SQLiteDatabase?) {
        val sql : String = "CREATE TABLE moviexuser(idMoviexUser int primarykey," +
                " idMovie integer," +
                " idUser integer)"
        db?.execSQL(sql)
    }
    ...
    {
        val sql : String = "SELECT movies.idMovie, movies.name, movies.synopsis, movies.duration, movies.year, movies.score, movies.director, movies.image FROM moviexuser JOIN movies USING (idMovie) INNER JOIN users WHERE users.idUser = '$id'"
